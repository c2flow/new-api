...
func RelaySwapFace(c *gin.Context, info *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	if billingApplied {
		...
		model.UpdateUserUsedQuotaAndRequestCount(info.UserId, midjourneyTask.Quota, info.OrgId)
...
func RelayMidjourneySubmit(c *gin.Context, relayInfo *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	if relayInfo.OrgId > 0 && consumeQuota {
	...
	if billingApplied {
		...
		model.UpdateUserUsedQuotaAndRequestCount(relayInfo.UserId, midjourneyTask.Quota, relayInfo.OrgId)
		model.UpdateChannelUsedQuota(billingChannelId, midjourneyTask.Quota)
	}
