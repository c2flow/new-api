...
func UpdateOrganizationMember(orgID, actorID, userID int, role string, status int) error {
	if (role != OrgRoleAdmin && role != OrgRoleMember) || (status != OrganizationActive && status != OrganizationDisabled && status != OrganizationDeleting) {
	...
	return DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(&member).Updates(map[string]interface{}{"role": role, "status": status}).Error; err != nil {
			return err
		}
