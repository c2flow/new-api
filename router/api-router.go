...
func SetApiRouter(router *gin.Engine) {
	...
	setOrganizationRoutes(apiRouter)
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			selfRoute.Use(middleware.UserAuth(), middleware.OrganizationContext())
			{
				...
				selfRoute.GET("/topup/self", middleware.RequireSelectedOrgPermission("org.billing", "read"), controller.GetUserTopUps)
				selfRoute.POST("/topup", middleware.CriticalRateLimit(), middleware.RequireSelectedOrgPermission("org.billing", "write"), controller.TopUp)
				selfRoute.POST("/pay", middleware.CriticalRateLimit(), middleware.RequireSelectedOrgPermission("org.billing", "write"), controller.RequestEpay)
				selfRoute.POST("/amount", middleware.RequireSelectedOrgPermission("org.billing", "write"), controller.RequestAmount)
				selfRoute.POST("/stripe/pay", middleware.CriticalRateLimit(), middleware.RequireSelectedOrgPermission("org.billing", "write"), controller.RequestStripePay)
				selfRoute.POST("/stripe/amount", middleware.RequireSelectedOrgPermission("org.billing", "write"), controller.RequestStripeAmount)
				selfRoute.POST("/creem/pay", middleware.CriticalRateLimit(), middleware.RequireSelectedOrgPermission("org.billing", "write"), controller.RequestCreemPay)
				selfRoute.POST("/waffo/amount", middleware.RequireSelectedOrgPermission("org.billing", "write"), controller.RequestWaffoAmount)
				selfRoute.POST("/waffo/pay", middleware.CriticalRateLimit(), middleware.RequireSelectedOrgPermission("org.billing", "write"), controller.RequestWaffoPay)
				selfRoute.POST("/waffo-pancake/amount", middleware.RequireSelectedOrgPermission("org.billing", "write"), controller.RequestWaffoPancakeAmount)
				selfRoute.POST("/waffo-pancake/pay", middleware.CriticalRateLimit(), middleware.RequireSelectedOrgPermission("org.billing", "write"), controller.RequestWaffoPancakePay)
		...
		tokenRoute.Use(middleware.UserAuth(), middleware.OrganizationContext())
		{
			tokenRoute.GET("/", middleware.RequireSelectedOrgPermission("org.token", "read"), controller.GetAllTokens)
			tokenRoute.GET("/search", middleware.RequireSelectedOrgPermission("org.token", "read"), middleware.SearchRateLimit(), controller.SearchTokens)
			tokenRoute.GET("/auto-groups", middleware.RequireSelectedOrgPermission("org.token", "read"), controller.GetTokenAutoGroups)
			tokenRoute.GET("/:id", middleware.RequireSelectedOrgPermission("org.token", "read"), controller.GetToken)
			tokenRoute.POST("/:id/key", middleware.RequireSelectedOrgPermission("org.token", "write"), middleware.CriticalRateLimit(), middleware.DisableCache(), controller.GetTokenKey)
			tokenRoute.POST("/", middleware.RequireSelectedOrgPermission("org.token", "write"), controller.AddToken)
			tokenRoute.PUT("/", middleware.RequireSelectedOrgPermission("org.token", "write"), controller.UpdateToken)
			tokenRoute.DELETE("/:id", middleware.RequireSelectedOrgPermission("org.token", "write"), controller.DeleteToken)
			tokenRoute.POST("/batch", middleware.RequireSelectedOrgPermission("org.token", "write"), controller.DeleteTokenBatch)
			tokenRoute.POST("/batch/keys", middleware.RequireSelectedOrgPermission("org.token", "write"), middleware.CriticalRateLimit(), middleware.DisableCache(), controller.GetTokenKeysBatch)
		}
		...
		logRoute.GET("/self/stat", middleware.UserAuth(), middleware.OrganizationContext(), middleware.RequireSelectedOrgPermission("org.usage", "read"), controller.GetScopedLogStats)
		logRoute.GET("/channel_affinity_usage_cache", middleware.AdminAuth(), controller.GetChannelAffinityUsageCacheStats)
		logRoute.GET("/search", middleware.AdminAuth(), controller.SearchAllLogs)
		logRoute.GET("/self", middleware.UserAuth(), middleware.OrganizationContext(), middleware.RequireSelectedOrgPermission("org.usage", "read"), controller.GetScopedLogs)
		logRoute.GET("/self/search", middleware.UserAuth(), middleware.OrganizationContext(), middleware.RequireSelectedOrgPermission("org.usage", "read"), middleware.SearchRateLimit(), controller.GetScopedLogs)
		...
		dataRoute.GET("/self", middleware.UserAuth(), middleware.OrganizationContext(), middleware.RequireSelectedOrgPermission("org.usage", "read"), controller.GetUserQuotaDates)
		dataRoute.GET("/flow", middleware.AdminAuth(), controller.GetAllFlowQuotaDates)
		dataRoute.GET("/flow/self", middleware.UserAuth(), middleware.OrganizationContext(), middleware.RequireSelectedOrgPermission("org.usage", "read"), controller.GetUserFlowQuotaDates)
